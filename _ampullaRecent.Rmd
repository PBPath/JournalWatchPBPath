

---

```{r ampulla 32646369, include=FALSE}

PMID_32646369 <- RefManageR::ReadPubMed('32646369', database = 'PubMed')

citation_32646369 <- paste0(PMID_32646369$journal,' ', PMID_32646369$year, ' ', PMID_32646369$month,';', PMID_32646369$volume,'(', PMID_32646369$number,'):', PMID_32646369$pages)

PubMed_32646369 <- paste0(PubMedString, PMID_32646369$eprint)

doi_32646369 <- paste0(doiString, PMID_32646369$doi)

Sys.sleep(2)

```

<details open> <summary>
- **`r PMID_32646369$title`**
</summary> 

*`r citation_32646369`*

`r PubMed_32646369`

`r PMID_32646369$abstract`

`r doi_32646369`

</details>

---




---

```{r ampulla 32618656, include=FALSE}

PMID_32618656 <- RefManageR::ReadPubMed('32618656', database = 'PubMed')

citation_32618656 <- paste0(PMID_32618656$journal,' ', PMID_32618656$year, ' ', PMID_32618656$month,';', PMID_32618656$volume,'(', PMID_32618656$number,'):', PMID_32618656$pages)

PubMed_32618656 <- paste0(PubMedString, PMID_32618656$eprint)

doi_32618656 <- paste0(doiString, PMID_32618656$doi)

Sys.sleep(2)

```

<details open> <summary>
- **`r PMID_32618656$title`**
</summary> 

*`r citation_32618656`*

`r PubMed_32618656`

`r PMID_32618656$abstract`

`r doi_32618656`

</details>

---




---

```{r ampulla 32171280, include=FALSE}

PMID_32171280 <- RefManageR::ReadPubMed('32171280', database = 'PubMed')

citation_32171280 <- paste0(PMID_32171280$journal,' ', PMID_32171280$year, ' ', PMID_32171280$month,';', PMID_32171280$volume,'(', PMID_32171280$number,'):', PMID_32171280$pages)

PubMed_32171280 <- paste0(PubMedString, PMID_32171280$eprint)

doi_32171280 <- paste0(doiString, PMID_32171280$doi)

Sys.sleep(2)

```

<details open> <summary>
- **`r PMID_32171280$title`**
</summary> 

*`r citation_32171280`*

`r PubMed_32171280`

`r PMID_32171280$abstract`

`r doi_32171280`

</details>

---




---

```{r ampulla 31612756, include=FALSE}

PMID_31612756 <- RefManageR::ReadPubMed('31612756', database = 'PubMed')

citation_31612756 <- paste0(PMID_31612756$journal,' ', PMID_31612756$year, ' ', PMID_31612756$month,';', PMID_31612756$volume,'(', PMID_31612756$number,'):', PMID_31612756$pages)

PubMed_31612756 <- paste0(PubMedString, PMID_31612756$eprint)

doi_31612756 <- paste0(doiString, PMID_31612756$doi)

Sys.sleep(2)

```

<details open> <summary>
- **`r PMID_31612756$title`**
</summary> 

*`r citation_31612756`*

`r PubMed_31612756`

`r PMID_31612756$abstract`

`r doi_31612756`

</details>

---


